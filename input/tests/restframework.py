...
class InputSubmissionTests(RestFrameworkTestCase):
    ...
    def test_submit(self):
        self.assertEqual(self.submit({'instrument': 1, 'data': 'foo'}), status.HTTP_201_CREATED)

    def test_submit_bad_collector_is_rejected(self):
        self.assertEqual(self.submit({'instrument': 1, 'data': 'foo', 'collector': 'fake'}), status.HTTP_400_BAD_REQUEST)

    def test_submit_bad_instrument_is_rejected(self):
        self.assertEqual(self.submit({'instrument': 2, 'data': 'foo'}), status.HTTP_400_BAD_REQUEST)

    def test_submit_bad_data_is_rejected(self):
        self.assertEqual(self.submit({'instrument': 1, 'data': {'foo': 'bar'}}), status.HTTP_400_BAD_REQUEST)
    ...
    def test_submit_planned_multiple_data_without_matching_input_model_is_rejected(self):
        ...
        self.assertEqual(self.submit({'instrument': 2, 'data': ['foo', 'bar']}), status.HTTP_400_BAD_REQUEST)
    ...
    def test_submit_policy_restrict_rejects_custom(self):
        ...
        self.assertEqual(self.submit({'instrument': 3, 'data': 'baz'}), status.HTTP_400_BAD_REQUEST)
        self.assertEqual(self.submit({'instrument': 3, 'data': 'foo'}), status.HTTP_201_CREATED)

    def test_submit_too_many_inputs_for_user_is_rejected(self):
        user = User.objects.create(username='user')
        self.client.force_login(user)
        self.assertEqual(self.submit({'instrument': 1, 'data': 'foo'}), status.HTTP_201_CREATED)
        self.assertEqual(self.submit({'instrument': 1, 'data': 'bar'}), status.HTTP_403_FORBIDDEN)
    ...
    def test_submit_too_many_inputs_is_rejected(self):
        ...
        self.client.force_login(user)
        self.assertEqual(self.submit({'instrument': 1, 'data': 'bar'}), status.HTTP_201_CREATED)

        user = User.objects.create(username='user3')
        self.client.force_login(user)
        self.assertEqual(self.submit({'instrument': 1, 'data': 'baz'}), status.HTTP_403_FORBIDDEN)
